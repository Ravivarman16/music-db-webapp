...
WITH ins1 AS (
        ...
        SELECT media_id, title, song_len FROM ins1
        )
        ,ins5 AS (
        INSERT INTO mediaserver.metadata (md_type_id,md_value)
        SELECT md_type_id, song_genre
        ...
        SELECT media_id, artist_id FROM ins1
        )
        , ins8 AS (INSERT INTO mediaserver.MediaItemMetaData
        SELECT media_id, md_id FROM ins1, ins2)
        
        -- RETURN LAST SONG'S ID (MAX)
		SELECT max(song_id)+1 as song_id FROM mediaserver.song;
